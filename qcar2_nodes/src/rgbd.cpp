...
class RGBD: public rclcpp::Node
{
    ...
    RGBD(): Node("rgbd")
    {
        // Initialize last valid frames with empty matrices
        last_valid_color_matrix = cv::Mat();
        last_valid_depth_matrix = cv::Mat();
    ...
    void publishImages()
    {
        ...
        if ((buffer_rgb != NULL) && (buffer_depth != NULL))
	    {
            ...
            if (result >= 0)
            {
                ...
                if (result >= 0)
                {
                    ...
                    if (!color_matrix.empty())
                    {
                        ...
                        color_matrix.copyTo(last_valid_color_matrix);
            ...
            else
            {
                ...
                // Only publish if we have a valid last frame
                if (!last_valid_color_matrix.empty())
                {
                    msg = cv_bridge::CvImage(hdr, sensor_msgs::image_encodings::BGR8, last_valid_color_matrix).toImageMsg();
                    msg->header.stamp = this->get_clock()->now();
                    msg->header.frame_id = "color_image";
                    rgb_pub_.publish(*msg);
                }
                ...
                {
                    RCLCPP_WARN(this->get_logger(), "No valid RGB frame available for publishing");
                }
            }
            ...
            if (result >= 0)
            {
                ...
                if (result >= 0)
                {
                    ...
                    {
                        // Store last valid depth frame
                        depth_matrix.copyTo(last_valid_depth_matrix);
                ...
                video3d_frame_release(depth_frame);
            ...
            else
                    {
                        ...
                        // Only publish if we have a valid last frame
                        if (!last_valid_depth_matrix.empty())
                        {
                            msg = cv_bridge::CvImage(hdr, sensor_msgs::image_encodings::MONO16, last_valid_depth_matrix).toImageMsg();
                            msg->header.stamp = this->get_clock()->now();
                            msg->header.frame_id = "depth_image";
                            depth_pub_.publish(*msg);
                        }
                        ...
                        {
                            RCLCPP_WARN(this->get_logger(), "No valid depth frame available for publishing");
                        }
                    }
    ...
    rcl_interfaces::msg::SetParametersResult set_parameters_callback(const std::vector<rclcpp::Parameter> & parameters)
    {
        ...
        // Keep the last valid frames as class members
        cv::Mat last_valid_color_matrix;
        cv::Mat last_valid_depth_matrix;
