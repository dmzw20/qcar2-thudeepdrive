...
class PathPlannerNode(Node):
    ...
    def create_control_window(self):
        """创建一个用于接收键盘输入的控制窗口"""
        # 创建一个小型控制窗口
        control_img = np.ones((200, 400, 3), dtype=np.uint8) * 240  # 浅灰色背景
        
        # 在窗口上显示使用说明
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(control_img, "QCar2 Path Planner", (10, 30), font, 0.7, (0, 0, 0), 2)
        cv2.putText(control_img, "Press 'p' to plan a new path", (10, 70), font, 0.6, (0, 0, 0), 1)
    ...
    def plan_and_publish_path(self):
        ...
        # 让用户选择目标点
        goal_point = get_user_click_position("Choose End Point", self.map_image, "目标点")
        if goal_point is None:
            self.get_logger().error("未选择目标点")
            return
        
        # 确保位置在可通行区域内
        vehicle_position = vehicle_pose["position"]
        y, x = vehicle_position
        
        # 确保坐标在图像范围内
        if not (0 <= y < self.traversable.shape[0] and 0 <= x < self.traversable.shape[1]):
            self.get_logger().error(f"车辆位置 ({y}, {x}) 超出地图范围")
            return
        
        if not self.traversable[vehicle_position]:
            self.get_logger().warning(f"车辆位置 ({y}, {x}) 不在可通行区域内")
            # 尝试寻找附近的可通行区域
            found = False
            for dy in range(-10, 11):
                for dx in range(-10, 11):
                    ny, nx = y + dy, x + dx
                    if (0 <= ny < self.traversable.shape[0] and 
                        0 <= nx < self.traversable.shape[1] and
                        self.traversable[ny, nx]):
                        vehicle_pose["position"] = (ny, nx)
                        self.get_logger().info(f"已将车辆位置调整为附近的可通行区域: ({ny}, {nx})")
                        found = True
                        break
                if found:
                    break
            if not found:
                self.get_logger().error("无法找到附近的可通行区域")
                return
        
        # 确保目标点可通行
        if not self.traversable[goal_point]:
            self.get_logger().error("目标点不在可通行区域内")
            return
        ...
        if path:
            ...
            cv2.imshow("Path Planning Result", result_image)
            
            # 发布路径到ROS话题
            self.publish_path(path, vehicle_pose)
            self.get_logger().info("路径规划完成并发布")
        else:
            self.get_logger().error("无法找到路径")
