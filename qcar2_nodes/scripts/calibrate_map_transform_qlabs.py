...
def visualize_calibration(map_image, pixel_points, world_points, transformation_matrix):
    ...
    # 保存可视化结果
    cv2.imwrite("calibration_visualization_qlabs.png", visual_img)
    print("已保存标定可视化结果到 calibration_visualization_qlabs.png")

def save_transformation_matrix(transformation_matrix, output_file="map_transform_qlabs.yaml"):
    """保存转换矩阵到YAML文件"""
    # 将转换矩阵转换为可序列化的格式
...
def main():
    ...
    # 收集标定点
    pixel_points, world_points = collect_calibration_points(map_image, num_points=4)
